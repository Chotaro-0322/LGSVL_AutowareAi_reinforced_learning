...
class Environment(Node):
    def __init__(self):
        ...
        self.num_states = 3 # ニューラルネットワークの入力数
    ...
    def run(self):
        ...
        for episode in range(NUM_EPISODES):
            ...
            state = np.array([self.current_pose.pose.position.x, 
                                        self.current_pose.pose.position.y, 
                                        self.current_pose.pose.position.z]) #####ここにposition(x, y, z), rotation(x, y, z), IMU(x, y, z)など様々な状態を(要素数)に並べる
            ...
            while self.closest_waypoint < (len(self.waypoint) - 1 - 3) and self.simulation_stop_flag == False: # 1エピソードのループ ※len(waypoint)はwaypointの総数だが, 0インデックスを考慮して -1 その次に 経路の終わりから3つ前までという意味で -3
                ...
                observation_next, done, error_dist = self.check_error()
                ...
                if done: # simulationが止まっていなかったらFalse, 終了するならTrue
                    ...
                    state_next = observation_next #####ここにposition(x, y, z), rotation(x, y, z), IMU(x, y, z)など様々な状態を(要素数)に並べる
